...
private fun AlertCard() {
  ...
  val animatedElevation: Dp by
      infiniteElevationAnimation.animateValue(
          initialValue = 1.dp,
          targetValue = 8.dp,
...
private fun AlertItem(message: String) {
  Row(
      modifier =
          Modifier.padding(RallyDefaultPadding)
              ...
              // properties of the descendants will be merged. If we'd use clearAndSetSemantics
              // instead,
      ...
      horizontalArrangement = Arrangement.SpaceBetween) {
        ...
        IconButton(onClick = {}, modifier = Modifier.align(Alignment.Top).clearAndSetSemantics {}) {
...
private fun <T> OverViewDivider(data: List<T>, values: (T) -> Float, colors: (T) -> Color) {
  Row(Modifier.fillMaxWidth()) {
    data.forEach { item: T ->
      Spacer(modifier = Modifier.weight(values(item)).height(1.dp).background(colors(item)))
    }
  }
}
...
private fun SeeAllButton(onClick: () -> Unit) {
  TextButton(onClick = onClick, modifier = Modifier.height(44.dp).fillMaxWidth()) {
